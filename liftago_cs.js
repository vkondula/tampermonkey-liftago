// ==UserScript==
// @name         Liftago rides counter (czech email)
...
$(document).ready(function()  {
    ...
    setTimeout(function(){
        var parse_message = function(message) {
            var regex = "([0-9.]+) ([A-Z]+).*DĚKUJEME ZA VAŠI JÍZDU NA (.*) PROFIL[^0-9]*([0-9]{1,2}\.([0-9]{1,2})\.[0-9]{4} [0-9]{2}:[0-9]{2})";
            var parsed = message.match(regex);
            if(parsed === null) return null;
            return {"currency": parsed[2], "price": parseFloat(parsed[1]), "type":parsed[3] ,"id":parsed[4], "month":parseInt(parsed[5])};
        };
        ...
        for (var i = 0; i < elements.length; i++) {
            if (elements[i].innerHTML.indexOf("DĚKUJEME ZA VAŠI JÍZDU") !== -1){
